...
(deftest user-examples
  ...
  (is (= {:type :item :bare {:type :integer :value 42} :params []}
         (sfv/parse-item "42")))
  (is (= "42" (sfv/serialize-item (sfv/parse-item "42"))))

  ;; Display String
  (is (= {:type :item :bare {:type :dstring :value "Grüße"} :params []}
         (sfv/parse-item "%\"Gr%c3%bc%c3%9fe\"")))
  (is (= "%\"Gr%c3%bc%c3%9fe\"" (sfv/serialize-item (sfv/parse-item "%\"Gr%c3%bc%c3%9fe\""))))
  ...
  (is (= "%\"%d8%a7%d9%84%d8%b3%d9%84%d8%a7%d9%85 %d8%b9%d9%84%d9%8a%d9%83%d9%85\""
         (sfv/serialize-item (sfv/parse-item "%\"%d8%a7%d9%84%d8%b3%d9%84%d8%a7%d9%85 %d8%b9%d9%84%d9%8a%d9%83%d9%85\""))))

  ;; Dates
  (is (= {:type :item, :bare {:type :date, :value 1659578233}, :params []}
         (sfv/parse-item "@1659578233")))
  (is (= "@1659578233" (sfv/serialize-item (sfv/parse-item "@1659578233"))))
...
;; Items can have params attached
  ...
  (is (= "pear;sweet" (sfv/serialize-item (sfv/parse-item "pear;sweet"))))
  ...
  (is (= "apple, pear;sweet=true, orange" (sfv/serialize-list (sfv/parse-list "apple, pear;sweet=true, orange"))))
  ...
  (is (= "max-age=3600, must-revalidate" (sfv/serialize-dict (sfv/parse-dict "max-age=3600, must-revalidate"))))
  ...
  (is (= "trees=(\"spruce\";type=conifer \"oak\";type=deciduous)"
         (sfv/serialize-dict (sfv/parse-dict "trees=(\"spruce\";type=conifer \"oak\";type=deciduous)"))))
  ...
  (is (= "foods=(\"burger\";sandwich \"pizza\";sandwich=?0 \"hot dog\";sandwich);comprehensive=?0"
         (sfv/serialize-dict (sfv/parse-dict "foods=(\"burger\";sandwich \"pizza\";sandwich=?0 \"hot dog\";sandwich);comprehensive=?0"))))
  ...
  (is (= "circle;color=red, square;filled=?0, (triangle;size=3 rectangle;size=4)"
         (sfv/serialize-list (sfv/parse-list "circle;color=red, square;filled=?0, (triangle;size=3 rectangle;size=4)")))))
  ...
  (testing "Parse and serialize Signature-Input headers from RFC 9421"
    ...
    (testing "Minimal signature with empty covered components"
      (let [input "sig-b21=();created=1618884473;keyid=\"test-key-rsa-pss\";nonce=\"b3k2pp5k7z-50gnwp.yemd\""
        ...
        (is (= input (sfv/serialize-dict parsed)))))
      ...
      (is (= {:type :dict
              :entries [["sig1" {:type :inner-list
                                 ...
                                 :params [["created" {:type :integer :value 1618884475}]
                                          ["keyid" {:type :string :value "test-key-rsa-pss"}]]}]]}
             parsed-input))
      (is (= signature-input (sfv/serialize-dict parsed-input)))
      (is (= signature (sfv/serialize-dict parsed-sig))))))
